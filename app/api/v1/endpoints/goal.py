...
def create_goal(goal_in: schemas.GoalCreate, db: Session = Depends(deps.get_db), current_user: models.User = Depends(deps.get_current_active_user)):
    return crud.goal.create_with_user(db=db, obj_in=goal_in, user=current_user)

@router.put("/{id}", response_model=schemas.Goal)
def update_goal(id: int, goal_in: schemas.GoalUpdate, db: Session = Depends(deps.get_db), current_user: models.User = Depends(deps.get_current_active_user)):
